...
(function () {
    ...
    var ALLOWED_TYPES = ['flick', 'puff', 'bud', 'bolt', 'ceph', 'rime', 'lantern'];
    ...
    function isHappyOnly(name) {
        return !!(ICE_HAPPY_ONLY[name] || PLANT_HAPPY_ONLY[name] || LANTERN_HAPPY_ONLY[name]);
    }
        ...
        window.hatchAnimation = function () {
            ...
            window.setHatchedPetType = function (t) {
                if (locked == null) {
                    ...
                    locked = t;
                    if (Math.random() < 1 / 6) {
                        locked = (Math.random() < 0.5) ? 'rime' : 'lantern';
                    }
                }
                innerSet(locked);
            };
    ...
    console.log('[pets.js] Pico PNG overrides ready; edge flood-fill chroma; 3 stages; NN upscale; hatch one line; lantern');
})();
